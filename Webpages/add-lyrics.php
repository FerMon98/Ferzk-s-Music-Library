...
function is_sel($a, $b){ return ((string)$a === (string)$b) ? 'selected' : ''; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <?php if (file_exists(__DIR__ . '/../components/head.php')) { include __DIR__ . '/../components/head.php'; } ?>
  <link rel="stylesheet" href="/Resources/CSS/style.css">
  <title>Add / Edit Lyrics</title>
  <style>
    .panel{ background: rgba(0,0,0,.35); border:1px solid rgba(255,255,255,.2); border-radius:12px; padding:1rem; }
    .stack{ display:flex; flex-direction:column; gap:.75rem; }
    .row{ display:flex; gap:.75rem; align-items:center; flex-wrap:wrap; }
    .wide{ width:min(1000px, 95%); margin: 1.25rem auto; }
    select, textarea, input[type=url]{
      width:100%; padding:.7rem .8rem; border-radius:10px; border:1px solid rgba(255,255,255,.35); color:#111; background:#fff;
    }
    textarea{ min-height: 40vh; resize: vertical; white-space: pre-wrap; }
    .actions{ display:flex; gap:.75rem; justify-content:flex-end; }
    .btn-primary{ padding:.6rem 1rem; border-radius:10px; border:2px solid darkblue; background:darkblue; color:#fff; font-weight:700; cursor:pointer; }
    .btn-primary:hover{ background:#ddd; color:#000; }
    .flash{ margin:.5rem 0; padding:.6rem .9rem; border-radius:10px; }
    .flash.ok{ background:rgba(0,180,0,.15); border:1px solid rgba(0,180,0,.35); color:#e7ffe7; }
    .flash.err{ background:rgba(255,0,0,.15); border:1px solid rgba(255,0,0,.35); color:#ffdede; }
